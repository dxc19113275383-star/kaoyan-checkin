...
- **已修（SW v21）**：`mcMountVideo` 加 `<video>`/HLS `error` 处理——视频加载失败（多为未上传 404）不再一片黑，改为清晰提示 + 重试 + 新窗口打开。
- 进行中：数学课程"章节 → 看视频 → **配套做题**"打通——做题侧题库已有（`zyx-gaoshu`/`zyx-xiandai`），尚未从课程页联动。
- 待办候选：把 course.json 与 R2 实际对象做覆盖率核对/裁剪（只登记已上传的）；填 `logic` / 长难句占位题集；校对 zyx 清洗题。
