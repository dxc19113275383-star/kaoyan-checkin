...
- **播放器按钮栏布局修复（2026-07-02）**：加「配套练习」后 4 个 `flex:1` 按钮挤一行，窄屏文字换行/裁切错位（"看上去废了"）。已改为两行——导航行（上一节/下一节）+ 操作行（标记已学/配套练习）并加 `white-space:nowrap`。见 `index.html` 的 `mcPlay()`（约 4780 行）。
