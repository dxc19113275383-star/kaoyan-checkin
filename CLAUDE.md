...
- **P1.2 阅读配套题已上线（2026-07-02）**：6 篇 ×4 题手写入各 passage JSON `questions` 字段；`renderPassage` 尾部渲染练习区、`readingAnswer()` 判分（答错入错题中心/重做答对解除/成绩存 `progress[id].quiz`，列表显「题 x/4」）。阅读闭环补全。下一步 P1.3 长难句复习队列 / P1.5 逻辑题库（需题源）。
