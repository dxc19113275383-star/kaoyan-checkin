...
│   ├── lib/
│   │   ├── storage/        # storageKeys / localStorageClient / storageTypes / appStateStore
...
用户操作 → 模块 Store（checkinStore 等）→ appStateStore.readSlice/writeSlice
        → localStorageClient → localStorage（键 ky_app_state_v7）
...
### 现役应用与工程层如何共存（不冲突）
- 现役 `index.html` 继续读写旧主状态键 **`kaoyan_v2`**（数值版本链，自带 `migrate()`）。
- V7.0 工程层读写**新键 `ky_app_state_v7`**（语义化 `AppState` 信封）。
- `bootstrap.ts` 在现役应用里**只读 `kaoyan_v2`、写新键 + 备份键**，全程 try/catch，
  因此与现役 `load()/save()` 互不干扰，迁移失败也不影响现役运行。
