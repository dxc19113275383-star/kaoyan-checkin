...
### Fixed
- **数学课程播放器按钮栏布局修复**：上次加「配套练习」后变成 4 个 `flex:1` 按钮挤一行，窄屏（约 360px）每个仅剩 ~75px，「标记已学 / 配套练习」文字换行被 40px 固定高裁切、图标错位，看上去像「废了」。改为两行分组——导航行（上一节 / 下一节）+ 操作行（标记已学 / 配套练习），并给按钮加 `white-space:nowrap`。无配套练习章节第二行仅「标记已学」占满，禁用态半透明正常。
