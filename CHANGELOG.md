...
### Added
- **背单词/阅读/视频/口语一批优化（用户 7 项）**：
  1. **有道 TTS 扩到句子**：例句与阅读朗读原来句子走 Edge 神经语音（被拦截就跌到系统合成音、发平无语气），现在英文单词+短句（≤240 字符）一律**有道 TTS 优先**（真人音库、国内快而稳、有语气），失败无缝回退 Edge/系统；朗读倍速透传到有道音频；新增有道预取减少句间停顿。
  2. **视频小窗（画中画）**：视频加 `autopictureinpicture` + 播放器「小窗」按钮 + 退到后台自动尝试进入小窗（`mcPip`，iOS 用 `webkitSetPresentationMode`、标准用 `requestPictureInPicture`；能否后台自动小窗取决于 iPhone 系统「画中画」开关，附提示）。
  3. **倍速不再被后台重置**：iOS 从后台返回会把 `playbackRate` 复位为 1，现在在 loadedmetadata/play/playing/可见性恢复 时都重设为期望倍速（读实时设置，不锁初值）。
  4. **例句逐词可点查义**：语境例句卡里的单词改为逐词可点（词形还原后查内置详解，未命中走 AI 兜底），目标词保持橙色高亮。
  5. **首页默认今天**：未设起始日期时，星期默认选中「今天」对应的星期，而不是永远停在周一。（顺带修 v9 遗留：数据自检里 `state.p>1` 硬编码会误伤 3+ 阶段，改为按 `PHASES.length` 判断。）
  6. **背单词混合模式**：模式加第三档「混合」——每张卡随机「看词选义」或「看释义默写」，复习不再只有选择题（选择固定到该卡不随重渲改变）。
  7. **新功能·口语练习（跟读）**：学习中心新增「口语练习」；4 组共 20 句常用句（日常/校园/出行/观点，内置离线），流程=听原声（有道）→ 麦克风录音（MediaRecorder）→ 回放对比；支持语音识别的设备额外给「语音测评」相似度打分（词重叠算法，iOS 不支持时自动隐藏、以录音自评为主）；练熟标记存 `state.learn.speak`。
- **热力图 v2（用户提出：与整体 UI 不协调 +「在1W」看不懂）**：配色从谷歌式红黄绿改为**品牌蓝三档浓度**（与液态玻璃主题一致），漏卡用极淡暖橙提示不刺眼，今天用品牌蓝描边圈；格子加大加圆角；行标签从「阶段首字+周数」（在1W）改为人话「本周/上周/N 周前」；图例精简为 无记录/漏卡/完成度渐变/今天。
- **首页阶段切换条撤除（用户提出：v9 后已无必要）**：阶段由起始日期自动定位、计划编辑在设置里，常驻大切换条只剩"翻看别的阶段"一个低频用途还占一整行、误点会把卡打到别的阶段。撤下后阶段切换收纳为周卡片里 `phaseLabel` 处的小下拉（多阶段才出现，单阶段纯文本），DOM 自检断言同步更新。
- **v9 计划引擎：从"考研专用"转型为通用学习助手（用户提出）**：
  - 写死的「在职打基础 32 周 / 全职冲刺 44 周」降级为内置默认模板（`BUILTIN_PLANS`）；`state.plans` 存用户自定义计划，`rebuildPhases()` 派生当前生效计划；迁移 v8→v9（`plans:null`=沿用内置、`goalLabel` 目标日可命名，如"雅思/期末考"）。
  - 全面泛化到任意 N 阶段：dateFor/calcPosition（起始日期自动定位）/monthIdxFor/热力图跨阶段回溯/周导航（上一周可退到上一阶段末周、下一周可进下一阶段）；阶段切换条按计划动态渲染、单阶段自动隐藏；阶段被删后当前位置自动收敛。
  - **设置抽屉新增计划编辑器**：每个阶段可改名/改周数/删除（至少保留一个，历史打卡数据保留）、可增阶段；「模板」展开编辑工作日/周六/周日任务清单（一行一个，可用"英语|/数学|/专业|/逻辑|/写作|/政治|/复习|"前缀映射科目标签）与阶段内每月目标；一键恢复默认考研双阶段模板。
  - 文案中性化：标题/manifest/PWA 名 → 燕燕学习助手；顶栏"燕燕学习·助手"；Hero 问候与 AI 上下文使用计划名；重规划建议去除阶段专属措辞；目标日倒计时提示可命名；"上岸"类考研黑话改中性。
  - **边界**：学习模块的内容（5500 词/阅读/长难句/作文/396 数学）仍是考研内容包，转型的是计划框架与产品定位；后续其他考试按"内容包"扩展。
  - 端到端验证：v9 迁移默认渲染与旧版一致；改名/加"雅思备考 12 周"阶段/自定义任务行标签解析/月目标/跨阶段周导航/自定义阶段打卡/恢复默认+越界收敛，自检 15/15。
- **P3 健壮与体验全部完成**：
