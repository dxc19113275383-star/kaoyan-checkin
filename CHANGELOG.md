...
### Added
- **P3 健壮与体验全部完成**：
  - `gen_*` 例句/翻译缓存加 LRU 上限（400 条，`gen__order` 维护次序；写入配额满时清最旧一半重试），不再无限吃 localStorage。
  - 自建词库/题库/文章导入超 900KB 弹体积确认（`_importSizeGuard`）；存储写满的报错改人话并给出清理建议。
  - 背单词首次加载大词库出骨架屏（"首次约 1 秒，之后秒开"），缓存命中直接进题。
  - iOS（网页/PWA）通知设置不再引导开启 Web Push（系统限制收不到），显示诚实说明并隐藏按钮，应用内提醒不受影响。
  - 周报升级：打卡页统计卡新增第二行「周背词 / 周做题 / 正确率 / 活跃天」（近 7 天学习脊柱 log 汇总，从尾部倒扫出窗即止，零学习数据时自动隐藏）。
- **学习中心重设计（用户提出：告别均匀方块阵）**：改为不对称 bento——「今日学习」深蓝渐变主横幅（本页唯一深色焦点）置顶；英语区 4/2 与 2/4 宽窄交错（背单词带今日进度条、阅读显已读篇数、长难句显待复习数、作文横排卡）；数学区 3/3；错题中心通栏细条（有存货显「N 待清」徽标、干净显"干净"）。每张卡都带真实数据，不再是死板图标+文案。**「资料/网课」入口撤下**（内容统一走 R2，页面与路由保留但无入口）；顺带确认首页快捷 bento 为历史死代码（宿主元素早已删除）。
### Fixed
- **视频课程弱网白屏（用户手机复现）**：移动网络下 `workers.dev`/`r2.dev` 可能长时间挂起而非快速失败，课程目录三级兜底链没有超时、页面又无加载态 → 白屏干等。修复：Worker 与 R2 直连两跳各加 6s 超时（`_fetchTimeout`，AbortController），最多 12s 必达同源内置目录；打开即显示骨架屏（附"弱网最多约 12 秒"说明）；`renderMcHome` 加 try/catch 兜底错误界面 + 重试按钮，任何情况不再白屏。
- **iOS 阅读/朗读没声音**：Safari 只允许"用户手势内"发起的音频播放，句子朗读是点击→异步合成→再播放，手势上下文丢失被静默拦截。修复：全局首次触摸播放无声 wav + 空 utterance 解锁音频会话（失败自动在下次手势重试），解锁后 Edge 神经语音/有道发音的程序化播放即可出声。注：若手机拨了静音键或音量为零仍无声，属系统行为。
- **无尺寸 SVG 图标在 iOS 上膨胀成巨图（用户截图报修 3 处）**：作文页「AI 批改」CTA、长难句「AI 详解」按钮里的机器人图标、范文「高分范文」标签里的书本图标撑爆容器。治本：9 个未带尺寸的图标定义（robot/book/folder/coach/spark/flame/pen/grid/sigma）补 `width/height="16"` 默认值 + 容器 CSS 兜底（`.btn-sm svg` 16px / `.btn-cta svg` 18px / `.fb-label svg` 14px / hero 火焰与教练图标 100% 填充不受影响）。与此前 `I.layers` 膨胀 186px 是同一类坑，此次一次清干净。
- **通知设置弹窗状态从不刷新（历史暗病）**：文件里存在两个同名 `updateNotificationUI`，后者操作早已不存在的 `#notifyStatus/#notifyPermBtn`、永远提前 return，却因函数提升遮蔽了真正更新弹窗的版本——弹窗状态一直显示写死的"未开启"。已删除死副本，弹窗现在能正确显示授权/订阅状态。
- **背单词四项优化（用户提出）**：
  - **满屏布局**：背单词 MCQ/拼写/完成卡、数学做题/完成卡、长难句单句/复习卡、今日学习页全部加 `fill-card` 占满视口（消除卡片下方大片空白）；单词区吃掉剩余高度垂直居中（`.fill-card > .vocab-card` flex:1）。
  - **选项即读**：点任一选项立即朗读该选项对应的单词（干扰项现在携带来源词 `data-w`）——答对读本词；**答错先读所选错误选项的词、再读正确的词**，听觉对比强化。
  - **发音升级**：`speak()` 重构——单个英文单词优先走**有道词典真人发音**（`dict.youdao.com/dictvoice`，美/英音自动选，国内网络稳、比合成音标准），加载失败/超时 2.5s 无缝回退 Edge 神经语音 → 系统语音；句子仍走神经语音链。
  - **语境例句卡（百词斩式）**：答题后反馈区新增深色渐变例句卡——例句中目标词橙色高亮 + 中文翻译 + 朗读按钮，数据来自现成的 5489 词义详解（`ex/exCn`），MCQ 与拼写模式都有。注：真·影视原声（剧照+原声音频）无版权素材源，暂以语境卡形似替代。
  - **选错揭示改版（用户提出）**：揭示后每个选项改为「英文单词大字在上（17px 粗体）+ 中文释义小字在下（12.5px）」，正确项的单词再放大到 21px——答案一眼锁定，不再是释义下缀小字。
  - **选错先看答案再看解析（用户提出）**：答错不再立刻弹词义详解糊脸——先停 1.6s 让用户看清绿色正确项与各选项单词（同时听"错词→对词"发音对比），再自动弹出词义详解；期间若已翻下一题则作废不弹。MCQ 与拼写模式一致。
  - **选错揭示选项单词（用户提出）**：MCQ 答错时，四个选项各自对应的英文单词显示在释义下方（错选红/正确绿/其余灰）——一眼看清把哪个词的意思记串了；答对不揭示保持干净。顺手修复连击下一题时旧朗读回调串音的竞态（翻题后作废"接着读正确词"的待发回调）。
  - **反馈溢出修复**：满屏化后答题反馈（语境卡+按钮）会把卡片撑出屏幕、「下一个」被底栏挡住——反馈渲染后自动平滑滚进视野（`_fbIntoView`，背单词两模式+数学做题），并给反馈区加 `scroll-margin-bottom` 避开固定底栏（scrollIntoView 感知不到浮层）。已验证溢出场景自动滚动后按钮完整可见。
- **沉浸阅读 v2（爱阅读式体验，用户提出）**：阅读文章页整体重排——
  - **排版**：英文改衬线 20px/1.95 大字（`--font-serif`），段距加大；标题下横幅「已为你标注「考研」词汇 N 个」。
  - **考研词自动标注**：文中命中本篇生词表的词=品牌蓝虚线下划线加粗（rd-key），命中考研 5500 大纲词的词=灰虚线（rd-hit，停用词表过滤 the/of 等高频功能词）；点词查义不变。
  - **每句「译」按钮**：句尾小圆钮独立开合该句译文（原全局"显示译文"保留在顶栏）。
  - **底部固定播放条**：上一句 / 播放暂停 / 下一句 / 倍速（0.8/1.0/1.2/1.5 循环，Edge 语音走百分比、系统语音走倍率，`speak()` 新增 `sysRate` 透传）；当前朗读句整句变品牌蓝（跟读高亮），暂停保留位置和高亮，上一句/下一句在暂停态只移动高亮。
  - 旧「试听全文/逐句喇叭」入口被播放条取代；配套练习/标记已读不变。已端到端验证（标注计数/句译开合/播放暂停跳句倍速/高亮样式）。
